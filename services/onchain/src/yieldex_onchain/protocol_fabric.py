...
class BaseProtocolOperator:
    ...
    def _load_contract(self) -> Contract:
        ...
        try:
            ...
            if self.protocol == 'fluid':
                self.contract_address = FLUID_ADDRESSES[self.network]['USDT'] ## Just for initial contract loading
    ...
    def _get_gas_params(self) -> Dict[str, Any]:
        ...
        if self.network == 'Arbitrum':
            ...
            logger.info(f"Using gas price for Arbitrum: {safe_gas_price} (base fee {base_fee})")
    ...
    def _send_transaction(self, tx_function) -> str:
        ...
        try:
            ...
            try:
                ...
                if "execution reverted" in error_str:
                    ...
                    if "message" in error_str:
                        ...
                        logger.error(f"Transaction would revert with reason: {revert_msg}")
                    else:
            ...
            if "Contract does not have fallback nor receive functions" in error_str:
                logger.error("This error typically occurs when trying to send native tokens (ETH) to a contract that doesn't accept them.")
                logger.error("Make sure you're not sending ETH value with your transaction to the ERC4626 vault.")
...
class FluidOperator(BaseProtocolOperator):
    ...
    def __init__(self, network: str, protocol: str = 'fluid'):
        ...
        if 'no_reserve_data_protocols' not in globals():
            no_reserve_data_protocols = ['yieldex-oracle', 'silo-v2', 'uniswap-v3', 'curve', 'fluid']
    ...
    def get_balance(self, token: str) -> float:
        ...
        try:
            ...
            if not self.dsa_manager or not self.dsa_connector:
                logger.warning("DSA not initialized, falling back to direct vault check")
    ...
    def supply(self, token: str, amount: float) -> str:
        ...
        try:
            ...
            # Check user balance
            user_balance_wei = token_contract.functions.balanceOf(self.account.address).call()
            user_balance = user_balance_wei / 10**decimals
            
            if user_balance < amount:
                logger.error(f"Insufficient {token} balance: {user_balance}, needed: {amount}")
                return None
    ...
    def withdraw(self, token: str, amount: float) -> str:
        ...
        try:
            ...
            if balance < amount:
                logger.error(f"Insufficient {token} balance in Fluid: {balance}, needed: {amount}")
                return None
...
def get_protocol_operator(network: str, protocol: str, **kwargs):
    ...
    if protocol_lower == 'aave-v3' or protocol_lower == 'aave-v2':
        ...
        logger.warning("⚠️ IMPORTANT: Fluid protocol requires specialized integration through DSA (DeFi Smart Account)")
        logger.warning("Direct RPC calls to Fluid vaults will NOT work - See documentation for details")
        logger.warning("Visit: https://docs.fluid.instadapp.io/integrate/understanding-fluid-key-concepts-and-practices.html")
        return FluidOperator(network, protocol_lower)
    else:
        supported = ['aave-v3', 'aave-v2', 'lendle', 'yieldex-oracle', 'curve', 'uniswap-v3', 'silo-v2', 'compound-v3', 'rho', 'fluid']
