...
def link_apy_history_to_pool_sites(supabase: Client) -> None:
    ...
    try:
        ...
        apy_history_response = supabase.table('apy_history').select('id,pool_id').is_('pool_site_id', 'null').limit(1000).execute()
        apy_records = apy_history_response.data
        
        logger.info(f"Found {len(apy_records)} records in apy_history without pool_site_id (limited to 1000)")
        ...
        for record in apy_records:
            ...
            if pool_id in pool_id_to_site_id:
                ...
                if not_found_count % 100 == 0:
                    logger.info(f"No matching pool_site for {not_found_count} records so far")
