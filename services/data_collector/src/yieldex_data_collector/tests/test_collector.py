...
from yieldex_data_collector.collector import fetch_pools, save_apy_data, run_data_collection, load_config, save_config, get_white_lists
import os
import json
...
def test_save_apy_data_with_extended_fields(mock_pools_response):
    ...
    with patch('data_collector.collector.create_client', return_value=mock_client), \
...
def test_save_apy_data_parametrized(pool_config):
    ...
    expected_pool_id = f"{base_id}_{pool_config['pool_meta']}" if "pool_meta" in pool_config else base_id
