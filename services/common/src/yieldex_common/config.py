...
def supply(self, token: str, amount: float) -> str:
    ...
    try:
        ...
        try:
            ...
            maxDeposit = fluid_vault_contract.functions.maxDeposit(self.account.address).call()
            logger.info(f"Max deposit allowed: {maxDeposit}")
            
            if amount_wei > maxDeposit:
                logger.warning(f"Amount exceeds max deposit limit. Reducing from {amount_wei} to {maxDeposit}")
