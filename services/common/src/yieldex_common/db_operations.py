...
def update_pool_balance(
    old_pool_id: str, new_pool_id: str, position_balance: float, tx_hash: str
) -> bool:
    """
    ...
    try:
        ...
        if not old_record:
            logger.error(
                f"Cannot update pool balance: No record found for {old_pool_id}"
            )
            return False
        ...
        if not result.data:
            logger.error(
                f"Failed to update pool balance for {old_pool_id} -> {new_pool_id}"
            )
            return False

        logger.info(
            f"Successfully updated pool balance: {old_pool_id} -> {new_pool_id} "
            f"(Amount: {position_balance}, TX: {tx_hash})"
        )
        return True
    except Exception as e:
        logger.error(
            f"Error updating pool balance {old_pool_id} -> {new_pool_id}: {str(e)}"
        )
        return False


def insert_pool_balance(
    pool_id: str, position_balance: float
    ...
    try:
        ...
        if result.data and len(result.data) > 0:
            logger.info(
                f"Successfully inserted pool balance for {pool_id} with {position_balance} USD"
            )
            return result.data[0]
        return None
    except Exception as e:
